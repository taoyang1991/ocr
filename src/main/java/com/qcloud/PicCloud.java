...
public class PicCloud {
    ...
    public IdCardInfoData idCardDetect(String url ,String card_type) {
        // create sign
        ...
        header.put("card_type", card_type);
//        header.put("Content-Length", "187");
        ...
        String reqUrl = "http://" + PROCESS_DOMAIN + "/ocr/idcard";
        JSONObject rspData = null;
...
//                setError(-1, "qcloud api response error");
        ...
        IdCardInfoData info = new IdCardInfoData();
        try {
            info.name = rspData.getString("name");
            info.sex = rspData.getString("sex");
            info.nation = rspData.getString("nation");
            info.birth = rspData.getString("birth");
            info.address = rspData.getString("address");
            info.id = rspData.getString("id");
        } catch (JSONException e) {
            setError(-1, "json exception, e=" + e.toString());
            return null;
        }
    ...
    public GeneralInfoData generalDetect(String url) {
        // create sign
        long expired = System.currentTimeMillis() / 1000 + 3600 * 24;
        String sign = getProcessSign(expired);
        if (null == sign) {
        ...
        String reqUrl = "http://" + PROCESS_DOMAIN + "/ocr/general";
        JSONObject rspData = null;
...
//                setError(-1, "qcloud api response error");
        ...
        GeneralInfoData info = new GeneralInfoData();
        try {
            JSONArray ja = rspData.getJSONArray("items");
            for(int i=0;i<ja.length();i++){
                JSONObject one = ja.getJSONObject(i);
                System.out.println(one.getString("itemstring"));
                info.item.add(one.getString("itemstring"));
            }
        } catch (JSONException e) {
            setError(-1, "json exception, e=" + e.toString());
            return null;
    ...
    public GeneralInfoData generalDetectFile(String[] pornFile) {
        // create sign
        long expired = System.currentTimeMillis() / 1000 + 3600 * 24;
        String sign = getProcessSign(expired);
        if (null == sign) {
        ...
        JSONObject rspData;
        try {
            String rsp = mClient.postgeneralfiles(reqUrl, header, body, data, pornFile);
    ...
    public ArrayList<IdCardInfoData> idcardDetectFile(String[] pornFile) {
        // create sign
        long expired = System.currentTimeMillis() / 1000 + 3600 * 24;
        String sign = getProcessSign(expired);
        if (null == sign) {
        ...
        String reqUrl = "http://" + PROCESS_DOMAIN + "/ocr/idcard";
        JSONObject rspData;
        System.out.println("== idcard body ==");
        ...
        ArrayList<IdCardInfoData> info = new ArrayList<IdCardInfoData>();
//        try {
//            JSONArray rl;
//            rl = rspData.getJSONArray("result_list");
//
